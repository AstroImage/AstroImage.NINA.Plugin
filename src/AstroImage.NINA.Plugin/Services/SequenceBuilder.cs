...
namespace AstroImage.NINA.Plugin.Services {
    ...
    public sealed class SequenceBuilder {
        ...
        public IDeepSkyObjectContainer? Costruisci(SequenceModel? modello, out Ricetta ricetta) {
             ...
             *  decisione gia' presa: la serie corta va in testa al suo gruppo, prima
             *  di quella lunga. Qui non si
             *  riordina niente. */
            var costruiti = 0;
        ...
        private ISequenceItem? Ripresa(RicettaBlocco b, int? ditherOgniPose,
            ...
            if (innesco is not null) {
                if (ditherOgniPose is int ogni && ogni > 0) {
                     ...
                     *  un valore di serie, e il motore lo legge — non lo calcola.
                     *
                     ...
                     *  un margine a ogni notte utile per fuoco, plate solve e calibrazione.
                     *  Qualche punto di ciclo utile ci sta
